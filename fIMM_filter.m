%%%%%%% fIMM_filter --- IMM filter based CV/CA models %%%%%%
function [Xgqk,Pqk,Mqk_1,Lk,Sk,vk]=fIMM_filter(Zk,Xgqk_1,Pqk_1,F_CV,G_CV,H_CV,F_CA,G_CA,H_CA,q,Rk,Ptc,Mqk_2)
...
Sk=zeros(3,3,2);
    % 模型交互
    for j=1:2
        Mhk_1(1,j)=Ptc(1,j)*Mqk_2(1,1)+Ptc(2,j)*Mqk_2(1,2); % 计算先验（交互后）模型概率1,2
    end
    for j=1:2
        for i=1:2
%             Ptrk_1(i,j)=Ptc(i,j)*Mqk_2(1,i)/Mhk_1(1,j); % 计算转入模型转移概率
            Ptrk_1(i,j)=Ptc(j,i)*Mqk_2(1,j)/Mhk_1(1,i); % 计算转入模型转移概率
        end
    end
    for j=1:2
%        Xghk_1(:,j)=Ptrk_1(1,j)*Xgqk_1(:,1)+Ptrk_1(2,j)*Xgqk_1(:,2);% 计算交互后状态估计  
        Xghk_1(:,j)=Ptrk_1(j,1)*Xgqk_1(:,1)+Ptrk_1(j,2)*Xgqk_1(:,2);% 计算交互后状态估计  
    end
    for j=1:2
        for i=1:2
            DP(:,:,i,j)=(Xgqk_1(:,i)-Xghk_1(:,j))*(Xgqk_1(:,i)-Xghk_1(:,j))'; % 与下式一起计算交互后协方差
        end
        Phk_1(:,:,j)=Ptrk_1(1,j)*(Pqk_1(:,:,1)+DP(:,:,1,j))+Ptrk_1(2,j)*(Pqk_1(:,:,2)+DP(:,:,2,j));  % 计算交互后协方差
    end
...
%             Pqk(9,9,i)=Xgqk(9,2)^2;
    ...
    for i=1:2
        Mqk_1(1,i)=Lk(1,i)*Mhk_1(1,i)/(Lk(1,1)*Mhk_1(1,1)+Lk(1,2)*Mhk_1(1,2)); % 计算后验（交互前）模型概率
        %Mq(i,k+1)=Mh(i,k);
    end
    %%% 全局模型加权输出
